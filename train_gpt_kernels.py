...
try:
    from triton_kernels import leaky_relu_squared, ste_int6_fakequant, fused_xsa
...
class CastedLinear(nn.Linear):
    # Use mutable containers so torch.compile cannot constant-fold them
    _qat_state: list[bool] = [False]
    _noisy_qat: list[bool] = [False]  # Noisy QAT: differentiable noise instead of STE

    @classmethod
    def set_qat_enabled(cls, enabled: bool, noisy: bool = False) -> None:
        cls._qat_state[0] = enabled
        cls._noisy_qat[0] = noisy

    @classmethod
    def is_qat_enabled(cls) -> bool:
        return cls._qat_state[0]

    def forward(self, x: Tensor) -> Tensor:
        w = self.weight.to(x.dtype)
        if self._qat_state[0] and self.training and w.ndim == 2:
            if self._noisy_qat[0]:
                # Noisy QAT: inject differentiable noise at int6 quantization step size
                # This makes the model robust to quantization without STE discontinuity
                with torch.no_grad():
                    amax = self.weight.float().abs().amax(dim=1, keepdim=True).clamp_min(1e-12)
                    step_size = amax / 31.0  # int6 quantization step size
                noise = (torch.rand_like(w) - 0.5) * step_size.to(w.dtype)
                w = w + noise  # Differentiable: gradients flow through
            else:
                # Standard STE fake-quantization
...
class Block(nn.Module):
    ...
    def forward(self, x: Tensor, x0: Tensor, q_w: Tensor, k_w: Tensor, v_w: Tensor, out_w: Tensor, up_w: Tensor, down_w: Tensor, v_embed: Tensor | None = None, v0: Tensor | None = None) -> tuple[Tensor, Tensor | None]:
        # Apply STE fake-quant to bank weights during QAT phase
        if self.training and CastedLinear._qat_state[0]:
            q_w = self._maybe_ste(q_w)
            k_w = self._maybe_ste(k_w)
            v_w = self._maybe_ste(v_w)
            out_w = self._maybe_ste(out_w)
            up_w = self._maybe_ste(up_w)
            down_w = self._maybe_ste(down_w)
...
def main() -> None:
    ...
    CastedLinear.set_qat_enabled(args.qat_enabled, noisy=args.noisy_qat)
    base_model = GPT(
    ...
    while True:
        ...
        if args.late_qat_threshold > 0 and scale < args.late_qat_threshold and not CastedLinear.is_qat_enabled():
            CastedLinear.set_qat_enabled(True, noisy=args.noisy_qat)
            log0(f"late_qat:enabled step:{step} scale:{scale:.4f} noisy:{args.noisy_qat}")
        zero_grad_all()
    ...
    # Temperature calibration: grid search for optimal logit temperature
    if args.eval_temperature > 0:
        log0(f"temperature:using fixed T={args.eval_temperature}")
        eval_model.logit_softcap = args.logit_softcap * args.eval_temperature
    elif args.eval_temperature == 0:
        # Auto-calibrate: try temperatures [0.85, 0.90, 0.95, 1.0] on a quick eval
        best_t, best_bpb = 1.0, float('inf')
        for t_cand in [0.85, 0.88, 0.90, 0.92, 0.95, 1.0]:
            eval_model.logit_softcap = args.logit_softcap * t_cand
            compiled_cal = torch.compile(eval_model, dynamic=False, fullgraph=True)
            cal_loss, cal_bpb = eval_val(
                args, compiled_cal, rank, world_size, device, grad_accum_steps,
                val_tokens, base_bytes_lut, has_leading_space_lut, is_boundary_token_lut,
                eval_seq_len=effective_eval_seq_len,
            )
            log0(f"  temp_calibration T={t_cand:.2f} val_bpb={cal_bpb:.6f}")
            if cal_bpb < best_bpb:
                best_t, best_bpb = t_cand, cal_bpb
        eval_model.logit_softcap = args.logit_softcap * best_t
        log0(f"temperature:auto best_T={best_t} best_bpb={best_bpb:.6f}")
    ...
    # Stride=16 evaluation (higher quality, slower)
    if args.eval_stride != 16 and 16 < sw_seq_len:
        ...
        torch.cuda.synchronize()
        log0(
            f"final_int6_sliding_window_s16 val_loss:{sw16_val_loss:.4f} val_bpb:{sw16_val_bpb:.4f} "
